...
;;; iNES Header
  ...
  .inesmir 1                    ; type of background mirroring: 0 = horizontal, 1 = vertical
...
.LoadBackground:
  ...
  ;; use combined ab as pointer to address of background
  LDA #LOW(background)
  STA m                         ; m <-> low byte
  LDA #HIGH(background)
  STA n                         ; n <-> high byte
...
.inner:
  ...
  INX                           ; x++
  CPX #$04                      ; x == 4 ?
  BNE .outer                    ; recur while x != 4 (run outer loop 4 times for a total of 32 rows)
...
SCROLL_RATE = $08               ; this should be something 256 is evenly divisble by so it will eventually loop back over to 0
HandleScrolling:
  LDA xPosition
  LDY scrollDirection
  CPY #SCROLL_RIGHT
...
.checkZero:
  ...
  BNE .finish
.stopScrolling:
  ;; toggle visibleScreen, change name table address in $2000
  LDA visibleScreen
  BEQ .screen1IsNewVisibleScreen
  LDY #$00
  STY visibleScreen
  LDA #%10010000
  JMP .set2000
.screen1IsNewVisibleScreen:
  LDY #$01
  STY visibleScreen
  LDA #%10010001
.set2000:
...
NMI:
  ...
  LDA scrollDirection
  BEQ .doHandleControllers
  BNE .doHandleScrolling
...
.cleanup:
  ...
  STA $2001
  LDA xPosition                 ; set background x scrolling offset
  STA $2005
  LDA #$00                      ; disable y scrolling
  STA $2005
